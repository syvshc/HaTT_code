function z = HBilinearForm(A, varargin)
  ...
  cr_x = x.core; cr_y = y.core; cr_a = A.core;
    ...
    for k = d - 1 : -1 : 1
      ...
      length_s = size(S, 1);
      W_L = zeros(r_a(k) * r_y(k), n(k) * length_s);
      W_R = zeros(r_x(k), n(k) * length_s);
      for i = 1 : n(k)
        for j = 1 : length_s
          ...
          W_L(:, (i - 1) * length_s + j) = reshape(Y1 * u * A1', [], 1);
          W_R(:, (i - 1) * length_s + j) = X1 * v;
